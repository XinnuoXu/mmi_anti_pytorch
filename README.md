...
In this step, we will train a language model based on the responses for the seq2seq model (example data `data/*.vi`). Since this language model will be used in the response generation model, it will share the dictionary (`data/*.vocab.pt`) generated by `Step 1`.
